...
//   console.log(movies)
  ...
  function searchMovies (title=" ", keyword= "") {
    const movieTittle = String(title).toUpperCase().trim()
    const searchValue = String(keyword).toUpperCase().trim()

    return movieTittle.includes(searchValue)
  }
...
//   console.log(searchMovies('insidious', '   IDI    '))
  ...
  function getMovies (keyword=''){
    ...
    for (let i=0; i<movies.length; i++) {
        let video = movies[i]
        let judul = video['title']
        let hasilSearch = searchMovies(judul,keyword)
        if (hasilSearch){
            result.push(video)
        }
    }
  ...
  // console.log(getMovies('the'))
//   console.log(movies[10]['title'])

function showMovies (keyword=''){
  let hasilTampil = getMovies (keyword)
  let movieHtml = document.getElementById('list-movies')
  let contentMovies =''
  for (let i =0; i< hasilTampil.length; i++){
    let movie = hasilTampil[i]
    contentMovies += `
                <div class="col">
                  <div class="card">
  ...
  movieHtml.innerHTML = contentMovies
}
